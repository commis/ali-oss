...
async function request(params) {
  ...
  try {
    // add extend header by brian
    _appendExtendSignHeader(reqParams, this.options);
...
function _appendExtendSignHeader(reqParams, options) {
  let urlObj = new URL(reqParams.url);
  let httpDate = dateformat(new Date(), 'UTC:ddd, dd mmm yyyy HH:MM:ss \'GMT\'');
  let signContent = [
    reqParams.params.method,
    urlObj.pathname,
    urlObj.searchParams.toString(),
    options.deviceId,
    httpDate,
    'x-user-agent:NodeJS-SDK'
  ];
  let canonicalString = signContent.join('\n') + '\n';
  let signature = signUtils.computeSignature(options.deviceSecret, canonicalString, options.headerEncoding);
